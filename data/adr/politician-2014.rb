...
module WVCrawler
	...
	class Crawler
		...
		def print_crawl_list
			...
			persist_to_mongo({:url => "http://www.adr.org/2014", 
					:candidate_list => ls2009candidates})
		end

		def persist_to_mongo(doc)
			if @use_mongo
				item = @coll.find_one(:url => doc[:url])
				if !(item)
					@coll.insert(doc)
				end
			end
		...
		def load_candidate_link_random
			if @use_mongo
				doc = @coll.find_one(:url => "http://www.adr.org/2014")
		...
		def crawl_candidate(url)
			...
			p["candidature"][0] = {}
			p["candidature"][0]["election"] = "Lok Sabha 2014"
			...
			if @constituencies[constituency_val]["loksabha-2014"].nil?
				@constituencies[constituency_val]["loksabha-2014"] = {}
				@constituencies[constituency_val]["loksabha-2014"]["candidate-name"] = []
			end
			@constituencies[constituency_val]["loksabha-2014"]["candidate-name"].push p["title"]
		...
		def get_candidate_wikipedia(candidate_name)
			...
			reference+= "- [Wikipedia profile]({{page.profile.wikipedia}}), accessed April 09, 2014.\n"
			if links.length > 3
...
=end
	...
	private
		...
		def clean_education(content)
			...
			char_to_delete = [":","-","\,"]
			details = content.gsub(level,"").strip.gsub(Regexp.union(char_to_delete),' ')
		...
		def get_track_record_section(url)
			...
			curr_objects.each do |election|
				...
				s+= "\n"
				if election_name != "lok sabha 2014"
					candidature = {}
...
c = WVCrawler::Crawler.new "http://myneta.info/ls2014/"
#c.get_links 
#c.print_crawl_list
